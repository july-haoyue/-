...
# 如果环境变量中没有设置API密钥，尝试从文件读取
if not api_key:
    try:
        with open('dashscope_key.txt', 'r', encoding='utf-8') as f:
            for line in f:
...
# 如果仍然没有获取到API密钥，使用默认值（仅用于测试）
if not api_key:
    api_key = 'sk-f29c379882c64a60b85f7f92a057689d'  # 从dashscope_key.txt中获取的密钥
    print("警告: 使用默认API密钥，请确保已设置环境变量DASHSCOPE_API_KEY")

# 设置dashscope的API密钥
dashscope.api_key = api_key
print(f"API密钥已设置: {'是' if api_key else '否'}")
...
def create_network_diagnosis_chain():
    ...
    try:
        ...
        try:
            ...
            print("使用备用模型 qwen-turbo 初始化成功")
        except Exception as e2:
            print(f"备用模型初始化也失败: {e2}")
            raise
